...
public class EMailItemVO {
    ...
    String title;
    //String content;
    int idx_mail_template;

    int row_num;

    long idx_email;//	number	32			◯		인덱스	auto increase
    int email_type;//	number	4				◯	이메일 타입, 발송조건,0 아니면 모두 예약	0직접작성 , 1:회원가입,2:공지사항알림, 3:개별일반, 개벌긴급,
    int schedule_type;//	number	4						0: 사용 안함, 1:즉시 전송, 2: 예약전송
    String subject;//	varchar2	200		0			이메일 제목	제목
    String content;//	clob							전송될 메세지, html
    ...
    String recipient;//	clob							"이름"<id@주소>; 형식
    int email_status;//	number	4					발송 상태	0:발송안함, 1:예약됨 2:발송함
    String memo;//	varchar2	1000					발송조건 설명	발송조건 설명
    int has_file;//	number	4					파일포함여부	0"포함 안함, 1:포함
    long idx_email_template;//	number	32
