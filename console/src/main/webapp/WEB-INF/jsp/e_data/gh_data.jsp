...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <h5 class="d-flex justify-content-between align-items-center w-100 mt-2">
        <div><i class="pe-7s-way text-success-custom"></i>시설 데이터</div>
    </h5>
                ...
                <div class="form-inline">
                    <div class="btn-group btn-group-sm btn-group-toggle srch_filtp1" data-toggle="buttons">
                        <label class="btn btn-secondary <c:if test="${filter1 eq 9999}">active</c:if>">
                            <input type="radio" name="btn-radio" class="category_change" value="0" checked onclick="javascript:pageLoad('gh_data',{cur_page:1,filter1:9999,filter2:9998},'시설 데이터');">최신
                        </label>
                            ...
                            <c:otherwise>
                                <c:forEach var = "i" begin = "1" end = "21">
                                    <c:if test="${i ne 12 and i ne 13}">
                                        <label class="btn btn-success <c:if test="${filter1 eq i}">active</c:if>">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <table id="table_demo_bs" class="table table-striped table-bordered dataTable no-footer mt-0" role="grid" aria-describedby="article-list_info" style="">
                            ...
                            <tr role="row">
                                <th class="text-center px-2" style="width:6%"><c:choose><c:when test="${filter1 ne 9999}">번호</c:when><c:otherwise>온실구역</c:otherwise></c:choose></th>
