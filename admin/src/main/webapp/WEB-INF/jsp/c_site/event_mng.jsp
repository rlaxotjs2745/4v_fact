    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card px-4 pt-2 mb-2 bg-">
            <div class="form-row mb-4">
                ...
                <div class="col-md-5 ">
                    <label class="form-label text-body">행사안내 상태별 보기</label>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <table id="table_demo_bs" class="table table-striped table-bordered dataTable no-footer mt-0" role="grid" aria-describedby="article-list_info" style="">
                                ...
                                <tbody>
                                <c:choose>
                                    <c:when test="${eventContentList eq null or eventContentList.size() eq 0}">

                                        <c:forEach var = "i" begin = "1" end = "4">
                                            <tr><td class="text-center empty-list" colspan="12"></td></tr>
                                        </c:forEach>
                                        <tr class=""><td class="text-center empty" colspan="12">내용이 없습니다</td></tr>
                                ...
                                <c:forEach items="${eventContentList}" var="item" varStatus="status">
                                <tr class="eventcontent_entity" id="${item.idx_event_content}">
                                    <td class="text-center">${item.row_num}</td>
                                    <td class="text-center"><a href="#none" class="btn btn-outline-default btn-sm btn_content_view" data-toggle="modal" data-target="#modals-counsel-view" data-idx="${item.row_num}">${item.subject}</a></td>
                                    <td class="text-center">${item.event_content_code}</td>
                                    ...
                                    <td class="text-center">${item.is_important eq 0 ? "고정안함" : item.is_important eq 1 ? "고정함" : "" }</td>
                                    <td class="text-center">${item.is_show eq 0 ? "노출안함" : item.is_show eq 1 ? "노출함" :""}</td>
                                    <td class="">${item.view_count}</td>
                                    <td class="text-center">${item.event_content_status eq 0 ? "임시저장" : item.event_content_status eq 1 ? "허락" : item.event_content_status eq 2 ? "수정발행" : item.event_content_status eq 3 ? "게시종료" : "기타"}</td>
                                    <td class="text-center">${item.memo}</td>
                                    <td class="text-center"><fmt:formatDate value="${item.event_start_date}" pattern="yyyy-MM-dd"/></td>
                                    <td class=""><fmt:formatDate value="${item.event_end_date}" pattern="yyyy-MM-dd"/></td>
                                </tr>
                                </c:forEach>
                                    </c:otherwise>
                                </c:choose>
                            ...
                            <div class="col-sm-12 col-md-7">
                                <div class="dataTables_paginate paging_simple_numbers" id="article-list_paginate">
                                    <ul class="pagination">
                                        ...
                                        <c:if test="${is_past eq true}"><li class="paginate_button page-item previous"><a href="javascript:pageLoad('event_mng',{cur_page:1,filter1:'${filter1}', filter2:'${filter2}'},'사업공고문 관리');" aria-controls="article-list" data-dt-idx="0" tabindex="0" class="page-link"><i class="fas fa-angle-double-left d-block"></i></a></li></c:if>
                                        <c:if test="${is_prev eq true}"><li class="paginate_button page-item previous"><a href="javascript:pageLoad('event_mng',{cur_page:'${cur_page-1}',filter1:'${filter1}',filter2:'${filter2}'},'사업공고문 관리');" aria-controls="article-list" data-dt-idx="0" tabindex="0" class="page-link"><i class="fas fa-angle-left d-block"></i></a></li></c:if>
                                        ...
                                        <c:if test="${is_next eq true}"><li class="paginate_button page-item next"><a href="javascript:pageLoad('event_mng',{cur_page:'${cur_page+1}',filter1:'${filter1}',filter2:'${filter2}'},'사업공고문 관리');" aria-controls="article-list" data-dt-idx="6" tabindex="0" class="page-link"><i class="fas fa-angle-right d-block"></i></a></li></c:if>
                                        <c:if test="${is_last eq true}"><li class="paginate_button page-item next"><a href="javascript:pageLoad('event_mng',{cur_page:'${tot_page}',filter1:'${filter1}',filter2:'${filter2}'},'사업공고문 관리');" aria-controls="article-list" data-dt-idx="6" tabindex="0" class="page-link"><i class="fas fa-angle-double-right d-block"></i></a></li></c:if>
                                    </ul>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-dialog modal-lg">
                <form class="modal-content">
                    <div class="modal-header bg-success">
                        <h5 class="modal-title text-white font-weight-bold">행사 안내</h5>
...
<script>
    ...
    var _page = 1;
    <c:forEach items="${eventContentList}" var="item" varStatus="status">
    eventContentList[${item.row_num}] = {
        idx_event_content: "${item.idx_event_content}",
        ...
        event_start_date:"<fmt:formatDate value="${item.event_start_date}" pattern="yyyy-MM-dd"/>",
        event_end_date:"<fmt:formatDate value="${item.event_end_date}" pattern="yyyy-MM-dd"/>",
        last_upd_date:"<fmt:formatDate value="${item.last_upd_date}" pattern="yyyy-MM-dd HH:MM"/>",
        event_content_status:"${item.event_content_status eq 0 ? "임시저장" : item.event_content_status eq 1 ? "허락" : item.event_content_status eq 2 ? "수정발행" : item.event_content_status eq 3 ? "게시종료" : "기타"}",
        event_content_status_no:${item.event_content_status},
    ...
    function _search(_p){
        ...
        pageLoad("c421_site_event_mng", {cur_page:_page,filter1:_filter1,filter2:_filter2}, "행사 관리",'site_event_mng');
    }
