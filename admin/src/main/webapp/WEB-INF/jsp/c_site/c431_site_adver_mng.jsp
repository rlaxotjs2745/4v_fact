...
<!-- Page content -->
                            ...
                            <table class="table table-bordered table-hover dataTable no-footer mt-0" role="grid" aria-describedby="article-list_info" style="">
                               ...
                               <c:forEach items="${prlist}" var="pr" varStatus="status">
                                <tr class="pr_entity" id="${pr.idx_pr_content}">
                                    <td class="text-center">${pr.row_num}</td>
                                    <td class="text-center"><a href="#none" id="data" class="btn btn-outline-default btn-sm btn_content_view" data-toggle="modal" data-target="#modals-counsel-view" data-idx="${pr.row_num}">${pr.subject}</a></td>
                                    <td class="text-center"><a href="#none" id="${pr.idx_pr_content}" class="btn btn-outline-default btn-sm btn_content_modal"  data-toggle="modal" data-target="#modals-content" data-idx="${pr.row_num}">컨텐츠 보기</a></td>
                        ...
                        <div class="col-sm-12 col-md-7">
                            <div class="dataTables_paginate paging_simple_numbers" id="article-list_paginate">
                                <ul class="pagination">
                                    ...
                                    <c:if test="${is_past eq true}"><li class="paginate_button page-item previous"><a href="javascript:pageLoad('c431_site_adver_mng',{cur_page:1},'홍보자료 목록','site_adver_mng');" aria-controls="article-list" data-dt-idx="0" tabindex="0" class="page-link"><i class="fas fa-angle-double-left d-block"></i></a></li></c:if>
                                    <c:if test="${is_prev eq true}"><li class="paginate_button page-item previous"><a href="javascript:pageLoad('c431_site_adver_mng',{cur_page:'${cur_page-1}'},'홍보자료 목록','site_adver_mng');" aria-controls="article-list" data-dt-idx="0" tabindex="0" class="page-link"><i class="fas fa-angle-left d-block"></i></a></li></c:if>
                                    <c:forEach var="i" begin="1" end="${page_amount}">
                                        <li class="paginate_button page-item <c:if test="${(cur_sector-1)*page_amount+i eq cur_page}">active</c:if>"><a href="javascript:pageLoad('c431_site_adver_mng',{cur_page:'${(cur_sector-1)*page_amount+i}'},'홍보자료 목록','site_adver_mng');" class="page-link">${(cur_sector-1)*page_amount+i}</a></li>
                                    </c:forEach>
                                    <c:if test="${is_next eq true}"><li class="paginate_button page-item next"><a href="javascript:pageLoad('c431_site_adver_mng',{cur_page:'${cur_page+1}'},'홍보자료 목록','site_adver_mng');" aria-controls="article-list" data-dt-idx="6" tabindex="0" class="page-link"><i class="fas fa-angle-right d-block"></i></a></li></c:if>
                                    <c:if test="${is_last eq true}"><li class="paginate_button page-item next"><a href="javascript:pageLoad('c431_site_adver_mng',{cur_page:'${tot_page}'},'홍보자료 목록','site_adver_mng');" aria-controls="article-list" data-dt-idx="6" tabindex="0" class="page-link"><i class="fas fa-angle-double-right d-block"></i></a></li></c:if>
                                </ul>
                            </div>
                        </div>
                    </div>
        ...
        <script>
            ...
            prList[${item.row_num}] = {
                ...
                is_new:"${item.is_new eq 0 ? "신규아님" : pr.is_new eq 1 ? "신규" :""}",
                is_file:"${item.is_file eq 0 ? "포함안함" : pr.is_file eq 1 ? "포함" :""}",
                is_main_page:${item.is_main_page},
                pr_content_code:"${v.pr_content_code}",
                ...
                pr_content_status:"${item.pr_content_status eq 0 ? "임시저장" :pr.pr_content_status eq 1 ? "허락" :item.pr_content_status eq 2 ? "수정발행": item.pr_content_status eq 3 ? "게시종료" :"기타"}",
