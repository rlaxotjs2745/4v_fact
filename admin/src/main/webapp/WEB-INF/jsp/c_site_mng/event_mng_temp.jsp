                ...
                <div class="col-sm-12">
                    <table id="" class="table table-bordered table-hover dataTable no-footer mt-0" role="grid" aria-describedby="article-list_info" style="">
                        ...
                        <tr class="eventcontent_entity" id="${eventcontent.idx_event_content}">
                            <td class="text-center">${eventcontent.row_num}</td>
                            <td class="text-center"><a href="#none" class="btn btn-outline-default btn-sm btn_content_view" data-toggle="modal" data-target="#modals-counsel-view" data-idx="${eventcontent.row_num}">${eventcontent.subject}</a></td>
                ...
                <div class="col-sm-12 col-md-7">
                    <div class="dataTables_paginate paging_simple_numbers" id="article-list_paginate">
                        <ul class="pagination">
                            ...
                            <c:if test="${is_past eq true}"><li class="paginate_button page-item previous"><a href="javascript:pageLoad('c421_site_event_mng',{cur_page:1},'행사 관리','site_event_mng');" aria-controls="article-list" data-dt-idx="0" tabindex="0" class="page-link"><i class="fas fa-angle-double-left d-block"></i></a></li></c:if>
                            <c:if test="${is_prev eq true}"><li class="paginate_button page-item previous"><a href="javascript:pageLoad('c421_site_event_mng',{cur_page:'${cur_page-1}'},'행사 관리','site_event_mng');" aria-controls="article-list" data-dt-idx="0" tabindex="0" class="page-link"><i class="fas fa-angle-left d-block"></i></a></li></c:if>
                            <c:forEach var="i" begin="1" end="${remain_page}">
                                <li class="paginate_button page-item <c:if test="${(cur_sector-1)*page_amount+i eq cur_page}">active</c:if>"><a href="javascript:pageLoad('c421_site_event_mng',{cur_page:${(cur_sector-1)*page_amount+i}},'행사 관리','site_event_mng');" class="page-link">${(cur_sector-1)*page_amount+i}</a></li>
                            </c:forEach>
                            <c:if test="${is_next eq true}"><li class="paginate_button page-item next"><a href="javascript:pageLoad('c421_site_event_mng',{cur_page:'${cur_page+1}'},'행사 관리','site_event_mng');" aria-controls="article-list" data-dt-idx="6" tabindex="0" class="page-link"><i class="fas fa-angle-right d-block"></i></a></li></c:if>
                            <c:if test="${is_last eq true}"><li class="paginate_button page-item next"><a href="javascript:pageLoad('c421_site_event_mng',{cur_page:'${cur_page}'},'행사 관리','site_event_mng');" aria-controls="article-list" data-dt-idx="6" tabindex="0" class="page-link"><i class="fas fa-angle-double-right d-block"></i></a></li></c:if>
                        </ul>
                    </div>
                </div>
            </div>
...
<script>
    ...
    eventContentList[${item.row_num}] = {
        ...
        event_content_code:"${item.event_content_code}",
        ...
        event_content_status:"${item.event_content_status eq 0 ? "임시저장" : item.event_content_status eq 1 ? "허락" : v.event_content_status eq 2 ? "수정발행" : item.event_content_status eq 3 ? "게시종료" : "기타"}",
        memo:"${item.memo}"
    }
    </c:forEach>
    _contents();
